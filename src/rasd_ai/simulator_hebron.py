...
class HebronCitySimulator:
    ...
    Columns: timestamp, tank_id, profile, level_pct, gas, temp_c, hum_pct
    """
    ...
    def generate(self) -> pd.DataFrame:
        ...
        for tank_id in range(self.cfg.n_tanks):
            ...
            # Effective daily accumulation (liters that actually build up)
            ...
            # Two hazard events (gas spikes) per tank over the month
...
# Example:
# df = HebronCitySimulator(HebronSimConfig()).generate()

if __name__ == "__main__":
    df = HebronCitySimulator(HebronSimConfig(n_tanks=20, days=30, freq_min=15)).generate()
    print(df.head())
